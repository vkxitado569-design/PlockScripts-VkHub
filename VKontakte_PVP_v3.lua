...
-- ============================================================
_G.SaveData = {}
local _hasFileIO = (writefile ~= nil and readfile ~= nil)
...
-- Safe key press wrapper: supports VirtualInputManager, keypress(), and fireclickdetector fallback
local function SafeSendKey(key)
    -- Method 1: VirtualInputManager (Synapse X, Fluxus, etc.)
    if vim1 then
    ...
    -- Method 2: keypress / keyrelease globals (older executors)
    if keypress and keyrelease then
        pcall(function()
            ...
            if code then keypress(code); task.wait(0.05); keyrelease(code) end
        end)
        return
    end
    -- Method 3: UserInputService simulation via writefile/loadstring if available (fallback)
    -- (no-op if nothing works — prevents nil errors)
end
...
-- Silent Aim metamethod hook (only works on executors with getrawmetatable/setreadonly)
    ...
    if AntiAFK then
        pcall(function()
            if vim2 then
                ...
                -- Fallback Anti-AFK: mover câmera levemente
                local cam = WS.CurrentCamera
                if cam then
                    local cf = cam.CFrame
