...
-- ANTI-DUPLICATA
-- ============================================================
if getgenv().VKontakteHub then
    pcall(function()
        for _, v in ipairs(game.CoreGui:GetChildren()) do
            if v.Name:find("VKontakte PvP") or v.Name:find("NazuX") then
                v:Destroy()
            end
        end
    end)
end
getgenv().VKontakteHub = true
...
if getgenv().Nousigi then 
	if game.CoreGui:FindFirstChild("VKontakte PvP GUI") then
		for i, v in ipairs(game.CoreGui:GetChildren()) do
			if string.find(v.Name,  "VKontakte PvP") then
				v:Destroy()
			end
		end
	end
...
Library_Function.Gui.Name = 'VKontakte PvP GUI'
...
Library_Function.NotiGui.Name = 'VKontakte PvP Notification'
...
ToggleScreenGui.Name = "VKontaktePvPToggle"
...
Library.ToggleUI = function()
    ...
    if game.CoreGui:FindFirstChild("VKontakte PvP GUI") then
        for a, b in ipairs(game.CoreGui:GetChildren()) do
            if b.Name == "VKontakte PvP GUI" then
...
Library.DestroyUI = function()
    ...
    local toggleGui = game.CoreGui:FindFirstChild("VKontaktePvPToggle")
    if toggleGui then
        toggleGui:Destroy()
    end
...
local libCreateNoti = function(Setting)
	...
	Ruafimg.Image = "rbxassetid://103324203833614"
    ...
    TextLabelNoti.Text = "<font color=\"rgb(" .. tostring(color or "255,80,80") .. ")\">" .. tostring(TitleNameMain or "VKontakte PvP") .. "</font> " .. tostring(getgenv().TitleNameNoti or "")
...
function Library:CreateWindow(Setting)
    local TitleNameMain = Setting.Title or "VKontakte PvP"
   ...
   TextLabelMain.Text = "<font color=\"rgb(0,0,0)\">" .. tostring(TitleNameMain or "VKontakte PvP") .. "</font> <font color=\"rgb(255,255,255)\">" .. tostring(getgenv().MainDesc or "") .. "</font>"
...
local LTitle   = MakeHUDLabel("🍌 VKONTAKTE PVP HUB v1.0", 4,  Color3.fromRGB(255, 210, 40))
...
local Window = Library:CreateWindow({
    Title    = "⚔️ VKontakte PvP Hub v1.0",
    Subtitle = "PVP Edition | Blox Fruits",
})

-- Fix mobile: garante que sliders inicializam corretamente
task.delay(0.5, function()
    for _, v in pairs(game:GetDescendants()) do
        if v:IsA("TextBox") and v.Name == "TextColor" and v.Text == "" then
            local parent = v.Parent
            if parent and parent.Name == "Background2" then
                -- slider sem valor inicial renderizado
            end
        end
    end
end)
...
local TpDelaySlider = SecTarget:AddSlider("TpDelay", {
...
task.delay(1, function() pcall(function() TpDelaySlider.SetValue(0.3) end) end)
...
local ComboDelaySlider = SecCombo:AddSlider("ComboDelay", {
    Text    = "Delay Combo (s)",
    Min     = 0.04,
    Max     = 0.5,
    Default = 0.08,
...
task.delay(1, function() pcall(function() ComboDelaySlider.SetValue(0.08) end) end)
...
local WalkSlider = SecDef:AddSlider("WalkSpeed", {
    Text    = "WalkSpeed",
    Min     = 16,
    Max     = 300,
    Default = 24,
...
task.delay(1, function() pcall(function() WalkSlider.SetValue(24) end) end)
...
local JumpSlider = SecDef:AddSlider("JumpPower", {
    Text    = "JumpPower",
    Min     = 50,
    Max     = 300,
    Default = 50,
...
task.delay(1, function() pcall(function() JumpSlider.SetValue(50) end) end)
...
local HuntSlider = SecHunt:AddSlider("HuntDelay", {
    Text    = "Delay Hunt (s)",
    Min     = 0.1,
    Max     = 1,
    Default = 0.25,
...
task.delay(1, function() pcall(function() HuntSlider.SetValue(0.25) end) end)
